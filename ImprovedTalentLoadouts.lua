...
local dataObj = LDB:NewDataObject(addonName, {
	type = "data source",
	text = "ITL: Spec, Loadout",
	OnClick = function(self)
		TalentLoadouts.easyMenuAnchor = self
		LibDD:ToggleDropDownMenu(1, nil, TalentLoadouts.easyMenu, self, 0, 0)
	end,
})
...
do
	...
	eventFrame:SetScript("OnEvent", function(self, event, arg1, arg2, arg3)
		...
		elseif event == "TRAIT_CONFIG_CREATED" and TalentLoadouts.blizzImported then
			if arg1.type ~= Enum.TraitConfigType.Combat then
				return
			end
		...
		elseif event == "ACTIVE_PLAYER_SPECIALIZATION_CHANGED" then
			...
			if not InCombatLockdown() then
				...
				delayed.UpdateActionBar = delayed.UpdateActionBar
					or GenerateClosure(TalentLoadouts.UpdateActionBar, TalentLoadouts)
				delayed.UpdateKnownFlyouts = delayed.UpdateKnownFlyouts
					or GenerateClosure(TalentLoadouts.UpdateKnownFlyouts, TalentLoadouts)
			end
		...
		elseif
			...
			and arg1 == "player"
			and arg3 == Constants.TraitConsts.COMMIT_COMBAT_TRAIT_CONFIG_CHANGES_SPELL_ID
		then
		...
		elseif
			...
			and TalentLoadouts.saveButton
			and TalentLoadouts.saveButton:IsShown()
		then
...
function TalentLoadouts:InitializeClassDBs()
	...
	for i, className in ipairs(classes) do
		ImprovedTalentLoadoutsDB.loadouts.globalLoadouts[className] = ImprovedTalentLoadoutsDB.loadouts.globalLoadouts[className]
			or { configIDs = {}, categories = {} }
	end
end
...
function TalentLoadouts:CheckDBIntegrity()
	ImprovedTalentLoadoutsDB.loadouts = ImprovedTalentLoadoutsDB.loadouts
		or { globalLoadouts = {}, characterLoadouts = {} }
...
function TalentLoadouts:CheckForDBUpdates()
	ImprovedTalentLoadoutsDB.options = ImprovedTalentLoadoutsDB.options
		or {
			...
			applyLoadout = ImprovedTalentLoadoutsDB.applyLoadout == nil and true
				or ImprovedTalentLoadoutsDB.applyLoadout,
			showSpecButtons = ImprovedTalentLoadoutsDB.showSpecButtons == nil and true
				or ImprovedTalentLoadoutsDB.showSpecButtons,
			specButtonType = "text",
		}
...
function TalentLoadouts:GetTreeID(configInfo)
	return (configInfo and configInfo.treeIDs[1])
		or (ClassTalentFrame and talentFrame:GetTalentTreeID())
		or (
			C_ClassTalents.GetActiveConfigID()
			and C_Traits.GetConfigInfo(C_ClassTalents.GetActiveConfigID()).treeIDs[1]
		)
end
...
function TalentLoadouts:UpdateTempLoadout()
	...
	for _, configID in ipairs(configIDs) do
		...
		if configInfo and configInfo.name == ITL_LOADOUT_NAME then
...
function TalentLoadouts:UpdateActionBar()
	if not ImprovedTalentLoadoutsDB.options.loadActionbarsSpec then
		return
	end
...
function TalentLoadouts:UpdateLoadoutIterator(categoryKey)
	...
	if ImprovedTalentLoadoutsDB.options.sortLoadoutsByName then
		iterateLoadouts = GenerateClosure(
			spairs,
			TalentLoadouts.globalDB.configIDs[currentSpecID] or {},
			sortByOrderAndName,
			categoryKey
		)
	else
		iterateLoadouts =
			GenerateClosure(spairs, TalentLoadouts.globalDB.configIDs[currentSpecID] or {}, sortByOrder, categoryKey)
	end
end
...
local function CreateEntryInfoFromString(configID, exportString, treeID, repeating)
	...
	if ClassTalentFrame then
		...
		success, loadoutEntryInfo =
			pcall(talentFrame.ConvertToImportLoadoutEntryInfo, talentFrame, configID, treeID, loadoutContent)
	elseif PlayerSpellsFrame then
		local _ = securecallfunction(
			PlayerSpellsFrame.TalentsFrame.ReadLoadoutHeader,
			PlayerSpellsFrame.TalentsFrame,
			importStream
		)
		loadoutContent = securecallfunction(
			PlayerSpellsFrame.TalentsFrame.ReadLoadoutContent,
			PlayerSpellsFrame.TalentsFrame,
			importStream,
			treeID
		)
		success, loadoutEntryInfo = pcall(
			PlayerSpellsFrame.TalentsFrame.ConvertToImportLoadoutEntryInfo,
			PlayerSpellsFrame.TalentsFrame,
			configID,
			treeID,
			loadoutContent
	...
	if specConfigIDs[currentSpecID] then
		for configID, configInfo in pairs(specConfigIDs[currentSpecID]) do
			if
				...
				and (not configInfo.exportString or not configInfo.entryInfo or not configInfo.treeHash)
			then
				configInfo.exportString, configInfo.entryInfo, configInfo.treeHash =
					CreateExportString(configInfo, configID, currentSpecID)
			end
		end
	else
		self:UpdateSpecID()
...
function TalentLoadouts:SaveLoadout(configID, currentSpecID)
	...
	if
		configInfo.type == 1
		and configInfo.name ~= ITL_LOADOUT_NAME
		and (not self.charDB.activeConfigIDs or not self.charDB.activeConfigIDs[configInfo.ID])
	then
...
function TalentLoadouts:SaveCurrentLoadouts()
	...
	if activeConfigID then
		self.globalDB.configIDs[currentSpecID][activeConfigID] = self.globalDB.configIDs[currentSpecID][activeConfigID]
			or C_Traits.GetConfigInfo(activeConfigID)
			...
			if pcall(C_Traits.GetConfigInfo, TalentLoadouts.charDB.tempLoadout) then
				...
				RunNextFrame(
					GenerateClosure(
						...
						TalentLoadouts.specID,
						TalentLoadouts.charDB.tempLoadout
					)
				)
			else
				--securecallfunction(talentFrame.UpdateTreeCurrencyInfo, talentFrame)
...
local function LoadLoadout(self, configInfo, categoryInfo, forceBlizzardDisable)
	...
	if not forceBlizzardDisable and ImprovedTalentLoadoutsDB.options.loadAsBlizzard then
		local tempLoadoutInfo = TalentLoadouts.charDB.tempLoadout
			and C_Traits.GetConfigInfo(TalentLoadouts.charDB.tempLoadout)
...
local function DeleteCategory(self, categoryInfo, withLoadouts)
	local dialog =
		StaticPopup_Show("TALENTLOADOUTS_CATEGORY_DELETE", withLoadouts and " (|cffff0000including loadouts|r)" or "")
...
function TalentLoadouts:SaveCurrentTree(loadoutName, categoryInfo)
	...
	if not isInspecting or exportString then
		return self:ImportLoadout(
			exportString or self:GetExportStringForTree(),
			loadoutName,
			categoryInfo and categoryInfo.key
		)
	elseif isInspecting and not exportString then
		TalentLoadouts:Print(
			"Unable to generate the export string. Make sure the specialization of the inspected unit is equalt to your current one."
		)
	end
end
...
local function UpdateWithCurrentTree(self, configID, categoryInfo, isButtonUpdate)
	if configID then
		...
		if configInfo then
			local activeConfigID = C_ClassTalents.GetActiveConfigID()
			configInfo.exportString, configInfo.entryInfo =
				CreateExportString(configInfo, activeConfigID, currentSpecID)
			configInfo.error = nil
...
local function SetCustomOrder(self, configID, categoryInfo)
	if configID then
		...
		if categoryInfo then
			local dialog =
				StaticPopup_Show("TALENTLOADOUTS_LOADOUT_CUSTOM_ORDER_CATEGORY", configInfo.name, categoryInfo.name)
...
StaticPopupDialogs["TALENTLOADOUTS_LOADOUT_IMPORT_NAME"] = {
	...
	OnHide = function(self, data)
		if self.action > 1 then
			...
			if not treeType or treeType == 1 then
				fakeConfigID =
					TalentLoadouts:ImportLoadout(importString, loadoutName, categoryInfo and categoryInfo.key)
			elseif treeType == 2 then
				loadoutName = string.format("[C] %s", loadoutName)
				fakeConfigID =
					TalentLoadouts:ImportClassLoadout(importString, loadoutName, categoryInfo and categoryInfo.key)
			elseif treeType == 3 then
				loadoutName = string.format("[S] %s", loadoutName)
				fakeConfigID =
					TalentLoadouts:ImportSpecLoadout(importString, loadoutName, categoryInfo and categoryInfo.key)
			end
	...
	if loadoutEntryInfo then
		...
		for i = 1, #loadoutEntryInfo do
			...
			if
				nodeCost
				and nodeCost[1]
				and C_Traits.GetTraitCurrencyInfo(nodeCost[1].ID) == Enum.TraitCurrencyFlag.UseSpecIcon
			then
	...
	if loadoutEntryInfo then
		...
		for i = 1, #loadoutEntryInfo do
			...
			if
				nodeCost
				and nodeCost[1]
				and C_Traits.GetTraitCurrencyInfo(nodeCost[1].ID) == Enum.TraitCurrencyFlag.UseClassIcon
			then
...
local function PostInChat(self, configID)
	...
	local configInfo = TalentLoadouts.globalDB.configIDs[currentSpecID][configID]
	if configInfo then
		local linkDisplayText = ("[%s - %s]"):format(
			TALENT_BUILD_CHAT_LINK_TEXT:format(PlayerUtil.GetSpecName(), PlayerUtil.GetClassName()),
			configInfo.name
		)
		local linkText = LinkUtil.FormatLink(
			"talentbuild",
			linkDisplayText,
			currentSpecID,
			UnitLevel("player"),
			configInfo.exportString
		)
...
function TalentLoadouts:LoadActionBar(actionBars, name)
	...
	if InCombatLockdown() then
		...
		delayed.LoadActionBar = delayed.LoadActionBar
			or GenerateClosure(TalentLoadouts.LoadActionBar, TalentLoadouts, actionBars, name)
		return
	end
		...
		if slotInfo then
			...
			elseif slotInfo.type == "macro" then
				if slotInfo.macroType and self[slotInfo.macroType] and not self.duplicates[slotInfo.macroName] then
					...
					elseif not id then
						self:Print(
							"Please resave your action bars. Couldn't find macro: ",
							slotInfo.macroName,
	...
	if printWarning then
		self:Print(
			"|cffff0000For now the AddOn won't place macros with duplicate names until Blizzard fixes the issue with receiving the macro body.|r"
		)
	end
end
...
function TalentLoadouts:AddSubCategoriesToImportDropdown(categoryInfo, currentSpecID, dropdownLevel, subCategoryLevel)
	if not categoryInfo.categories then
		return
	end
...
local function LoadoutDropdownInitialize(frame, level, menu, ...)
	...
	if level == 1 then
		...
		for configID, configInfo in iterateLoadouts() do
			if not configInfo.default and (not configInfo.categories or not next(configInfo.categories)) then
				...
				LibDD:UIDropDownMenu_AddButton({
					...
					value = configID,
					colorCode = color,
					text = customOrder and string.format("|cFFFFFFFF[%d]|r %s", customOrder, configInfo.name)
						or configInfo.name,
					hasArrow = true,
					minWidth = 170,
					fontObject = dropdownFont,
		...
		if frame == TalentLoadouts.easyMenu and not C_AddOns.IsAddOnLoaded(talentUI) then
			...
			LibDD:UIDropDownMenu_AddButton({
				...
				text = string.format(
					"%s %sNew Loadout|r",
					CreateAtlasMarkup("communities-icon-addchannelplus"),
	...
	elseif menu == "options" then
		...
		LibDD:UIDropDownMenu_AddButton({
			...
			fontObject = dropdownFont,
			func = function()
				ImprovedTalentLoadoutsDB.options.sortLoadoutsByName =
					not ImprovedTalentLoadoutsDB.options.sortLoadoutsByName
				TalentLoadouts:UpdateLoadoutIterator()
			end,
			checked = function()
		...
		LibDD:UIDropDownMenu_AddButton({
			...
			fontObject = dropdownFont,
			func = function()
				ImprovedTalentLoadoutsDB.options.sortCategoriesByName =
					not ImprovedTalentLoadoutsDB.options.sortCategoriesByName
				TalentLoadouts:UpdateCategoryIterator()
			end,
			checked = function()
		...
		LibDD:UIDropDownMenu_AddButton({
			...
			fontObject = dropdownFont,
			func = function()
				ImprovedTalentLoadoutsDB.options.sortSubcategoriesByName =
					not ImprovedTalentLoadoutsDB.options.sortSubcategoriesByName
			end,
			checked = function()
	...
	elseif menu == "actionBarOptions" then
		...
		LibDD:UIDropDownMenu_AddButton({
			...
			keepShownOnClick = 1,
			func = function()
				ImprovedTalentLoadoutsDB.options.loadActionbarsSpec =
					not ImprovedTalentLoadoutsDB.options.loadActionbarsSpec
			end,
			checked = function()
	...
	elseif menu == "globalLoadoutOptions" then
		...
		LibDD:UIDropDownMenu_AddButton({
			...
			keepShownOnClick = 1,
			func = function()
				ImprovedTalentLoadoutsDB.options.useAddOnLoadoutFallback =
					not ImprovedTalentLoadoutsDB.options.useAddOnLoadoutFallback
			end,
			checked = function()
		...
		LibDD:UIDropDownMenu_AddButton({
			...
			keepShownOnClick = 1,
			func = function()
				ImprovedTalentLoadoutsDB.options.findMatchingLoadout =
					not ImprovedTalentLoadoutsDB.options.findMatchingLoadout
			end,
			checked = function()
	...
	elseif menu == "loadout" then
		...
		for _, func in ipairs(arrowFunctions) do
			...
			if
				info
				and (not info.required or configInfo[info.required])
				and (not info.level or level == info.level)
			then
				LibDD:UIDropDownMenu_AddButton({
					arg1 = configID,
					arg2 = categoryInfo,
	...
	elseif menu == "category" then
		...
		if categoryInfo then
			if categoryInfo.categories then
				...
				local iterator =
					GenerateClosure(sort and spairs or ipairs, categoryInfo.categories, sort and sortByValue or nil)
			...
			for configID, configInfo in iterateLoadouts() do
				if categoryLoadouts[configID] ~= nil and configInfo and not configInfo.default then
					local customOrder = ImprovedTalentLoadoutsDB.options.showCustomOrder
						and configInfo.categoryCustomOrder
						and configInfo.categoryCustomOrder[L_UIDROPDOWNMENU_MENU_VALUE.key]
					LibDD:UIDropDownMenu_AddButton({
						arg1 = configInfo,
						arg2 = categoryInfo,
...
function TalentLoadouts:InitializeHooks()
	...
	hooksecurefunc(SlashCmdList, "ACECONSOLE_SIMC", function()
		if not ImprovedTalentLoadoutsDB.options.simc then
			return
		end

		local customLoadouts
		if ITLAPI.EnumerateSpecLoadouts and type(ITLAPI.EnumerateSpecLoadouts) == "function" then
...
local function CreateTextSpecButton(width, specIndex, _, specName)
	local specButton =
		CreateFrame("Button", nil, TalentLoadouts.dropdown, "UIPanelButtonNoTooltipTemplate, UIButtonTemplate")
...
local function CreateIconSpecButton(width, specIndex, numSpecializations, _, icon)
	...
	specButton:SetPoint(
		"LEFT",
		talentFrame.ResetButton,
		"RIGHT",
		(specIndex - 1) * (width / 2 + 1) + (width * (numSpecializations == 3 and 1.25 or 1)),
		-2
	)
	...
	if C_SpellBook and C_SpellBook.GetSpellBookItemType then
		for skillLineIndex = 1, C_SpellBook.GetNumSpellBookSkillLines() do
			...
			for i = 1, skillLineInfo.numSpellBookItems do
				local itemType, id, spellID =
					C_SpellBook.GetSpellBookItemType(i + skillLineInfo.itemIndexOffset, Enum.SpellBookSpellBank.Player)
